...
- Name: Chat - JumboIA >> CHAT-003: Limite de caracteres no input
- Location: C:\Users\monte\Desktop\Projetos\JumboIA\tests\specs\chat.spec.ts:38:7
...
Expected: <= 500
Received:    600
    at C:\Users\monte\Desktop\Projetos\JumboIA\tests\specs\chat.spec.ts:46:26
```
...
- main:
  - img
  - heading "Bem-vindo ao JumboIA" [level=1]
  - paragraph: Pergunte a JumboIA qualquer dúvida sobre suas matérias escolares e obtenha respostas detalhadas e precisas.
  - button "Explique o ciclo da água de forma simples"
  - button "Como resolver equações de segundo grau?"
  - button "Quais são os principais eventos da Segunda Guerra Mundial?"
  - textbox "Digite sua pergunta aqui...": 
  - text: 600/5000
  - button "JumboIA"
...
```ts
   ...
   2 |
   3 | // CHAT-001: Envio de mensagem simples
   4 | // CHAT-002: Manutenção de contexto em conversa
   5 | // CHAT-003: Limite de caracteres no input
   6 | // CHAT-004: Envio de mensagem vazia
   7 | // CHAT-005: Tratamento de erro da API OpenRouter (ex: chave inválida)
   8 | // CHAT-006: Tratamento de erro de rede ao enviar mensagem
   9 | // CHAT-007: Interação com prompts longos
   10 | // CHAT-008: Humanização do texto da resposta
   11 | // CHAT-009: Cancelamento de requisição (se implementado)
   12 |
   13 | test.describe('Chat - JumboIA', () => {
   14 |   test('CHAT-001: Envio de mensagem simples', async ({ page }) => {
   15 |     await page.goto('/');
   16 |     const input = page.locator('input[type="text"]');
   17 |     const sendButton = page.getByRole('button', { name: /enviar/i });
